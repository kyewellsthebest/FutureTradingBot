...
# Extended feature columns. Added in v2 of the schema for retrospective
# analytics and eventual ML filtering. ALL ARE NULLABLE -- they're populated
# best-effort at trade-open time. Missing values just mean that context
# wasn't available, not that the trade is invalid.
_V2_FEATURE_COLUMNS = [
    # Volatility / regime context
    ("regime",                  "TEXT"),
    ("atr_5_at_entry",          "REAL"),
    ("atr_60_at_entry",         "REAL"),
    ("vol_ratio",               "REAL"),
    # Strategy reasoning
    ("impulse_pts",             "REAL"),
    ("impulse_range_pts",       "REAL"),
    # Time / session
    ("session",                 "TEXT"),    # 'asia'/'london'/'us_open'/'us_lunch'/'us_close'/'overnight'
    ("et_hour",                 "INTEGER"),
    ("et_minute",               "INTEGER"),
    ("day_of_week",             "INTEGER"), # 0=Monday
    # News proximity
    ("news_minutes_until",      "INTEGER"),
    ("news_title",              "TEXT"),
    ("news_importance",         "TEXT"),
    # Cross-market state
    ("vix",                     "REAL"),
    ("vix_change_5m_pct",       "REAL"),
    ("nq_es_spread_5m_pct",     "REAL"),
    ("dxy_change_60m_pct",      "REAL"),
    ("macro_alignment_score",   "REAL"),
    # Session structure
    ("nearest_level",           "TEXT"),    # e.g. 'pdh', 'poc', 'vah', 'val'
    ("distance_to_level_pts",   "REAL"),
    ("in_value_area",           "INTEGER"), # bool
    ("above_pdh",               "INTEGER"), # bool
    ("vwap_distance_pts",       "REAL"),
    # Execution quality (filled at close time)
    ("slippage_pts",            "REAL"),
    ("latency_ms",              "REAL"),
    ("mae_pts",                 "REAL"),    # max adverse excursion
    ("mfe_pts",                 "REAL"),    # max favourable excursion
    # Risk gate context
    ("denied_by_gate",          "TEXT"),    # set ONLY for denied setups in a separate log
]


def migrate_v2_feature_columns() -> int:
    """Idempotent: adds the v2 feature columns to existing trades table if
    they don't exist. Safe to call on every startup. Returns count of
    columns added."""
    added = 0
    try:
        with _conn() as conn:
            existing = {row[1] for row in conn.execute("PRAGMA table_info(trades)").fetchall()}
            for col_name, col_type in _V2_FEATURE_COLUMNS:
                if col_name not in existing:
                    try:
                        conn.execute(f"ALTER TABLE trades ADD COLUMN {col_name} {col_type}")
                        added += 1
                    except sqlite3.OperationalError:
                        # Column already exists or DB locked -- non-fatal
                        pass
            conn.commit()
    except Exception:
        pass
...
def insert_trade_with_features(t: dict) -> int:
    """Like insert_trade(), but accepts ALL the v2 feature columns too.
    Unknown keys are silently ignored. Required keys: same as insert_trade
    (signal_name, side, entry_time, entry_px, stop_px, target_px, qty).

    This is the function fib_main should call after the v2 migration."""
    base_keys = ("signal_name", "side", "entry_time", "entry_px", "stop_px",
                 "target_px", "qty", "ml_decision", "ml_confidence",
                 "vol_regime", "daily_bias", "rr")
    v2_keys = tuple(c[0] for c in _V2_FEATURE_COLUMNS)
    all_keys = base_keys + v2_keys
    # Filter to known keys, allow None for v2 fields
    cols = [k for k in all_keys if k in t]
    if not cols:
        return 0
    values = tuple(t.get(k) for k in cols)
    placeholders = ", ".join("?" * len(cols))
    col_list = ", ".join(cols)
    with _conn() as conn:
        cur = conn.execute(
            f"INSERT INTO trades ({col_list}) VALUES ({placeholders})", values)
        return int(cur.lastrowid)


def update_trade_execution_stats(trade_id: int,
                                    slippage_pts: float | None = None,
                                    latency_ms: float | None = None,
                                    mae_pts: float | None = None,
                                    mfe_pts: float | None = None) -> None:
    """Update execution-quality fields on a trade row. Called on close
    once MAE/MFE are known and the actual fill price is recorded."""
    updates = []
    params = []
    for name, val in (("slippage_pts", slippage_pts),
    ...
    try:
        with _conn() as conn:
            conn.execute(f"UPDATE trades SET {', '.join(updates)} WHERE id = ?",
                          tuple(params))
    except Exception:
        pass
