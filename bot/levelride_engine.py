"""LEVELRIDE-LADDER 3-rung — PAPER-ONLY engine (v1).
...
weeks. THIS ENGINE PLACES NO BROKER ORDERS - it keeps its own paper
ledger in data_dir()/levelride_engine.json so live-fill reality can
be measured against sim before any real deployment (which requires
an explicit user go and a separate change).
"""
import json
import logging
import os
...
class LevelrideEngine:
    ...
    def snapshot(self) -> dict:
        return {
            "strategy": "LEVELRIDE-LADDER 3 (PAPER)",
