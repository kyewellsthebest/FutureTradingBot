...
def on_new_1m_bar(state: FibStrategyState, lucid: LucidState,
    ...
    for setup in state.pending_setups:
        ...
        # EXECUTABILITY GATE -- STRICTLY OPT-IN (unset by default).
        # 2026-07-03: briefly shipped with a 3pt default, which changed
        # WHICH trades paper takes -- that is the user's strategy, not
        # execution, and was reverted per user instruction. Paper fires
        # exactly as it always did unless STRAT_FIRE_DRIFT_GATE_PT is
        # explicitly set in the environment. When set: skip the fire
        # while current price is more than <gate> pts past the entry on
        # the favourable side (the setup stays pending and fires if
        # price returns -- resting-LIMIT semantics). Context for the
        # optional gate: the 2026-07-02 23:54 bundle measured median
        # +5pt / p90 +27pt favourable drift at open; such entries cannot
        # be filled by any broker order at the booked price.
        _gate_env = os.environ.get("STRAT_FIRE_DRIFT_GATE_PT", "").strip()
        if _gate_env:
            try:
...
def try_fire_on_tick(state: FibStrategyState, lucid: LucidState,
    ...
    for setup in state.pending_setups:
        ...
        # fill condition.
        # EXECUTABILITY GATE -- STRICTLY OPT-IN (unset by default; see
        # the bar-path note above). Paper's tick firing is unchanged
        # unless STRAT_FIRE_DRIFT_GATE_PT is explicitly set.
        _gate_env = os.environ.get("STRAT_FIRE_DRIFT_GATE_PT", "").strip()
        if _gate_env:
