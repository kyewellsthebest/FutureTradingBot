...
def streams():
    """FOCUSED SWEEP: short-lookback pullbacks, densely sampled.

    The broad search barely touched this corner -- five configs at an 8-unit
    lookback out of 3,353 -- and those five fired 8.8 times a week against a
    median of 2.6 and were positive in every holdout week. Short lookback is
    the only lever found so far that buys frequency without costing edge, so
    this grid samples it properly instead of stepping over it.

    Pullbacks past 1.0 are included deliberately: entering beyond a full
    ...
    for lb, k, pb in itertools.product(LB, K, PB):
        ...
        # the same trigger taken at market, to separate the mechanism from the
        # one-tick advantage a resting limit enjoys
        yield ("sfibM", dict(lb=lb, k=k, pb=pb), "M", i2, s2,
...
_SF = (dict(sp=[0.5, 1.0, 2.0], rr=[0.75, 1.0, 1.5], trail=[0.0, 2.5],
            ttl=[1, 3], H=[2, 6, 12, 24]) if DEPTH == 2 else
       dict(sp=[0.2, 0.3, 0.4, 0.6, 0.8, 1.0, 1.25, 1.5, 2.0, 2.5],
            rr=[0.4, 0.5, 0.618, 0.75, 1.0, 1.25, 1.5, 2.0],
            trail=[0.0, 1.5, 2.5], ttl=[1, 2, 3, 6],
            H=[1, 2, 3, 4, 6, 9, 12, 18, 24, 48]))
EXITS["sfib"] = _SF
EXITS["sfibM"] = _SF
...
SUF = ("d" if DEPTH == 3 else "b") + "_f" + SERIES + (f"_sh{SH_K}" if SHARD else "")
