...
while [ "$(date +%s)" -lt "$END" ]; do
  sleep "$INT"
  git add -A data/vsearch_state.json data/vsearch.log research/VSEARCH.md \
          data/mega6_state.json data/mega6.log research/MEGA6.md 2>/dev/null
  if ! git diff --staged --quiet; then
    n=$(python3 -c "
import json
try: d=json.load(open('data/vsearch_state.json')); print(f\"{len(d.get('winners',[])):,} validated\")
