...
Writes: s1_{ROOT}.json.gz (top configs + funnel stats), s1base_{ROOT}.npz
...
# ---------------------------------------------------------------- data prep
df = pd.read_csv(f"/home/user/FutureTradingBot/data/polygon/{ROOT}_5min.csv")
...
vix = pd.read_csv("/home/user/FutureTradingBot/data/polygon/vix_daily.csv")
...
# combo matrix (32 cells -> 108 filter combos); combo 0 = no filters
combos = []
CMAT = np.zeros((32, 108), dtype=np.float64)
ci = 0
        ...
        for vol_f in ("none", "lo", "hi"):
            for vx_f in ("none", "lo", "hi"):
                for cell in range(32):
                    s, t, v, x = cell >> 3, (cell >> 2) & 1, (cell >> 1) & 1, cell & 1
                    ok = True
                ...
                combos.append((sess_f, tr_f, vol_f, vx_f))
                ci += 1
...
def streams():
    ...
    for lb, k, pb in itertools.product([6, 12, 24, 48, 96], [1.0, 1.5, 2.0, 3.0],
    ...
    for lb, k, dep in itertools.product([6, 12, 24, 48], [0.8, 1.2, 1.8, 2.5],
                                        [0.25, 0.5, 1.0]):
        m = mom(lb)
        sig = sigok & np.isfinite(m) & (np.abs(m) > k * ATR)
        idx = thin(np.where(sig)[0])
    ...
    for N, buf in itertools.product([24, 48, 96, 192, 288], [0.0, 0.25]):
    ...
    for N, m_ in itertools.product([48, 96, 288], [1, 2]):
        dh, dl = donhi(N), donlo(N)
    ...
    for m_, buf in itertools.product([3, 6, 12], [0.0, 0.25]):
        sig_i, sides, pxs = [], [], []
    ...
    for p, buf in itertools.product([0.6, 0.75], [0.0, 0.25]):
    ...
    for hh, lb in itertools.product([7.0, 13.5, 23.0], [24, 72]):
        m = mom(lb)
    ...
    if ROOT in ("ES", "RTY", "YM", "GC", "CL"):
        ...
        gapv = O - prev_close
        for g, mode in itertools.product([0.3, 0.6, 1.0], ["fade", "go"]):
...
# ------------------------------------------------------------ scoring loop
EXITS = {
    "fib":      dict(sp=[0.75, 1.0, 1.5, 2.5], rr=[0.75, 1.0, 1.5, 2.5],
                     trail=[0.0, 2.5], ttl=[2, 3, 12], H=[12, 24, 48]),
    "fade":     dict(sp=[0.75, 1.0, 1.5, 2.0], rr=[0.75, 1.0, 1.5],
...
for fam, pdict, etype, idx, side, px in streams():
    ...
    cell = (sess_cat[idx] << 3) | (trend_with << 2) | (vol_hi[idx] << 1) | vix_hi[idx]
    for ttl in ex["ttl"]:
        ...
        for Hbars in ex["H"]:
            ...
            for sp_mult, rr, trail in itertools.product(ex["sp"], ex["rr"], ex["trail"]):
                ...
                cg = cello[good]
                wct = wk_codes[xb] * 32 + cg
                WCT = np.bincount(wct, weights=pnl, minlength=W * 32).reshape(W, 32)
                CNT = np.bincount(wct, minlength=W * 32).astype(np.float64).reshape(W, 32)
                wkP = WCT @ CMAT
                wkN = CNT @ CMAT
                trP = wkP[wk_tr_idx]
                ...
                GW = np.bincount(wct, weights=np.maximum(pnl, 0), minlength=W * 32).reshape(W, 32)
                GL = np.bincount(wct, weights=np.minimum(pnl, 0), minlength=W * 32).reshape(W, 32)
                WIN = np.bincount(wct, weights=(pnl > 0).astype(np.float64),
                                  minlength=W * 32).reshape(W, 32)
                dct = day_codes[xb] * 32 + cg
                DCT = np.bincount(dct, weights=pnl, minlength=ND * 32).reshape(ND, 32)
                DCN = np.bincount(dct, minlength=ND * 32).astype(np.float64).reshape(ND, 32)
                gi = np.where(gate)[0]
                ...
                for z, g in enumerate(gi):
                    ...
                    rows_all.append(dict(
                        ...
                        f_vol=combos[g][2], f_vix=combos[g][3],
...
if rows_all:
    ...
    score = (0.22 * pr(df_r.sharpe) + 0.14 * pr(df_r.ulcer, False) +
             0.14 * pr(df_r.maxdd) + 0.12 * pr(df_r.avg_lose_day) +
             0.12 * pr(df_r.worst_wk) + 0.08 * pr(df_r.o10) +
             0.06 * pr(np.log1p(df_r.n_tr)) + 0.06 * pr(df_r.pf) +
             0.03 * pr(df_r.ev) + 0.03 * pr(df_r.wk))
    df_r["score"] = score.round(4)
    df_r = df_r.sort_values("score", ascending=False).head(3000)
    top = df_r.to_dict("records")
else:
    top = []

out = dict(root=ROOT, traded_as=TRADED_AS, pv=PV, tick=TICK, comm=COMM,
           margin=MARGIN, afford=bool(AFFORD), engine="pessimistic-vectorized-v2",
...
with gzip.open(f"{D}/s1_{ROOT}.json.gz", "wt") as f:
    json.dump(out, f)
bd = pd.DataFrame(base_rows)
if len(bd):
    np.savez_compressed(f"{D}/s1base_{ROOT}.npz", data=bd.to_records(index=False))
print(f"{ROOT}: saved {len(top)} top rows, {len(base_rows)} base rows", flush=True)
