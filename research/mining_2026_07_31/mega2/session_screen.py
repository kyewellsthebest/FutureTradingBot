...
def main():
    ...
    L = ["# C1: is there directional skill outside US cash hours?", "",
         ...
         "| EV half | EV full |", "|" + "---|" * 8]
    for ev_f, ev_h, pt, tpd, sname, name, S, T, n in rows[:30]:
        L.append(f"| {sname} | {name} | {S:.0f}/{T:.0f} | {n:,} | "
                 f"{tpd:.0f} | {pt:.2%} | ${ev_h:+.2f} | ${ev_f:+.2f} |")

    L += ["", "## Per-session summary (best cell and RANDOM control)", "",
          "| session | best signal | best target-first | best EV full | "
          "RANDOM target-first |", "|" + "---|" * 5]
    for sname in SESSIONS:
        sub = [r for r in rows if r[4] == sname]
        if not sub:
            continue
        best = sub[0]
        rnd = [r for r in sub if r[5] == "RANDOM"
               and r[6] == 10.0 and r[7] == 20.0]
        L.append(f"| {sname} | {best[5]} {best[6]:.0f}/{best[7]:.0f} | "
                 f"{best[2]:.2%} | ${best[0]:+.2f} | "
                 f"{rnd[0][2]:.2%} |" if rnd else
                 f"| {sname} | {best[5]} | {best[2]:.2%} | "
                 f"${best[0]:+.2f} | n/a |")
    pos = [r for r in rows if r[0] > 0 and r[5] != "RANDOM"]
    L += ["", f"**Cells with positive EV at full-tick cost "
          f"(excluding RANDOM): {len(pos)} of {len(rows)}**", ""]
    for r in pos[:15]:
        L.append(f"- {r[4]} {r[5]} {r[6]:.0f}/{r[7]:.0f}: {r[2]:.2%} "
                 f"target-first, ${r[0]:+.2f}/trade, {r[3]:.0f} "
                 f"trades/day over {r[8]:,} signals")
    L.append("")
