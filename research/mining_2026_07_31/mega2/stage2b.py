...
Reads s1{SUF}_{ROOT}.json.gz; writes s2{SUF}_{ROOT}.json
"""
...
def fmask_for(idx, side, f_sess, f_trend, f_vol, f_vix, f_htf="none"):
    ...
    if f_htf == "up": m &= mod.htf_up[idx] == 1
    elif f_htf == "dn": m &= mod.htf_up[idx] == 0
    return m
...
def metrics(trades):
    ...
    # trade-order randomization: maxDD distribution over shuffled trade sequences
    pv = tr_.pnl.values
    sims2 = rs.permutation(np.tile(pv, (500, 1)).T).T if False else None
    dds2 = []
    for _ in range(500):
        p = rs.permutation(pv)
        c = np.cumsum(p)
        dds2.append((c - np.maximum.accumulate(c)).min())
    out["shuffle_dd_p95"] = round(float(np.percentile(dds2, 5)))
    # rolling 13-week windows: fraction positive
    if len(wk) >= 20:
        roll = wk.rolling(13).sum().dropna()
        out["roll13_pos"] = round(float((roll > 0).mean()), 3)
    else:
        out["roll13_pos"] = 0.0
    # regime slices on train trades
    et_idx = np.searchsorted(ts.values, pd.to_datetime(tr_.ei.values).values)
    et_idx = np.clip(et_idx, 0, n - 1)
    vh = mod.vol_hi[et_idx]; hu = mod.htf_up[et_idx]
    out["slice_vol_hi"] = round(float(tr_.pnl.values[vh == 1].sum()))
    out["slice_vol_lo"] = round(float(tr_.pnl.values[vh == 0].sum()))
    out["slice_htf_up"] = round(float(tr_.pnl.values[hu == 1].sum()))
    out["slice_htf_dn"] = round(float(tr_.pnl.values[hu == 0].sum()))
    # store weekly vector for correlation / portfolio work
    out["_wkvec"] = {k: round(float(v), 2) for k, v in wk.items()}
    return out

# --------------------------------------------------- neighbor cliff check
GRIDS = dict(
    fib=dict(lb=[6, 8, 12, 16, 24, 32, 48, 96], k=[0.75, 1.0, 1.5, 2.0, 3.0],
             pb=[0.236, 0.382, 0.5, 0.618, 0.706, 0.786, 0.886],
             sp=[0.75, 1.0, 1.5, 2.0, 2.5], rr=[0.5, 0.75, 1.0, 1.5, 2.5],
             ttl=[2, 3, 12], H=[12, 24, 48]),
    momcont=dict(lb=[6, 12, 24, 48], k=[1.0, 1.5, 2.0, 2.5],
                 sp=[0.75, 1.0, 1.5, 2.0], rr=[0.75, 1.0, 1.5, 2.0], H=[6, 12, 24]),
    exhaust=dict(r=[4, 5, 6, 8], k=[1.5, 2.5], sp=[0.75, 1.0, 1.5, 2.0],
                 rr=[0.75, 1.0, 1.5], H=[6, 12, 24]),
    fade=dict(lb=[4, 6, 12, 24, 48], k=[0.6, 0.8, 1.2, 1.8, 2.5], dep=[0.0, 0.25, 0.5, 1.0],
...
def cfg_stream_key(r):
    ...
    sig_keys = {"fib": ["lb", "k", "pb"], "fade": ["lb", "k", "dep"],
                ...
                "todmom": ["h", "lb"], "gap": ["g", "mode"], "momcont": ["lb", "k"], "exhaust": ["r", "k"]}[fam]
...
with gzip.open(f"{D}/s1{SUF}_{ROOT}.json.gz", "rt") as f:
    s1 = json.load(f)
top = s1["top"]
if not top:
    json.dump(dict(root=ROOT, n_selected=0, survivors=[], note="no stage-1 passers"),
              open(f"{D}/s2{SUF}_{ROOT}.json", "w"), indent=1)
...
try:
    bnp = np.load(f"{D}/s1{SUF}base_{ROOT}.npz", allow_pickle=True)["data"]
...
for r in sel:
    ...
    etype, idx, side, px, a_sig = smap[key]
    fm = fmask_for(idx, side, r["f_sess"], r["f_trend"], r["f_vol"], r["f_vix"], r.get("f_htf", "none"))
