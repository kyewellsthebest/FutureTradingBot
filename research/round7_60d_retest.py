"""Round 7 60-day re-test: take CANDIDATE strategy names from CLI and
run them under the same bot-faithful execution model on the FULL 60-day
window (offset 7_820_974_790). Writes round7_60d_results.md.

Usage:
  python3 round7_60d_retest.py --candidates name1,name2,name3
"""
...
def main():
    ...
    ap.add_argument("--out", default="/home/user/HFTBot/research/round7_60d_results.md")
    args = ap.parse_args()

    wanted = set(args.candidates.split(","))

    v1m, v15s, v30s, vtick, vquote, _ = r7.build_variants_round7()
    ...
    vquote = [s for s in vquote if s.name in keep_set]
    all_strats = v1m + v15s + v30s + vtick + vquote
    print(f"[60d] Re-testing {len(all_strats)} candidates on 60d", file=sys.stderr)
    if not all_strats:
        return
    ...
    with open(args.out, "w") as f:
        f.write(f"# Round 7 60-DAY re-test\n\n")
