#!/bin/bash
# Supervisor for the round8 strategy search.
# Restarts the python process whenever it dies, resuming from checkpoint.
# Usage:
#   nohup ./research/round8_supervisor.sh [offset] [suffix] [max_days] > /tmp/round8_supervisor.log 2>&1 &
# Stop:
#   pkill -f round8_supervisor.sh && pkill -f round8_search.py
set -u

SCRIPT="/home/user/HFTBot/research/round8_search.py"
OFFSET="${1:-8300000000}"
SUFFIX="${2:-}"
MAX_DAYS="${3:-14}"
RESULTS="/home/user/HFTBot/research/round8_results${SUFFIX}.md"
CHECKPOINT="/home/user/HFTBot/research/round8_checkpoint${SUFFIX}.pkl"
RUN_LOG="/tmp/round8_run${SUFFIX}.log"
MAX_BACKOFF_S=60
...
while true; do
    ...
    if [ ! -f "$CHECKPOINT" ] && [ -f "$RESULTS" ]; then
        ...
        if [ "$results_age" -lt 600 ]; then
            echo "[$(date '+%Y-%m-%d %H:%M:%S')] supervisor: round8 completed successfully. Exiting."
            exit 0
        fi
    fi
