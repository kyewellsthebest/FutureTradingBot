...
SRC = {}
for off in (0.0, 0.25, -0.25, 0.5, -0.5, 0.75, -0.75, 1.0, -1.0,
            1.25, -1.25, 1.5, -1.5, 1.75, -1.75, 2.0, -2.0, 2.5,
            -2.5, 3.0, -3.0, 3.5, -3.5, 4.0, -4.0):
    SRC[f"L{off:+.2f}"] = M["S"].shift(1) + off
S1 = M["S"].shift(1)

CFGS = []
for tg in (5., 10., 15., 25., 40., 60., 85., 115., 150., 200., 260., 300.):
    for st_ in (4., 6., 9., 12., 18., 25., 34., 46., 60., 80.):
        for hd in (2, 5, 15, 30, 60, 120, 240):
            for cf in (0,):
                for dr in ("b", "s", "l"):
                    for gt in ("all", "st", "sl"):
...
for src, lv in SRC.items():
    ...
    for ci in order:
        ...
        if shown >= 15:
            break
    np.savez(f"research/round83_{src}.npz", usd_wk=usd_wk,
