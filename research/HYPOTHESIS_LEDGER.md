...
| 34 | **needle: 1.2M strategies, pass/fail bar in ALL 8 contracts, symmetric controls** | 8 NQ contracts, 3 tape sets | 1,209,600 strategies x 2 arms | **NULL — real is 123x WORSE than the noise floor** | The user's fix to methodology: stop ranking (the best of N noise draws is sqrt(2 ln N) sigma, so ranking finds the luckiest), and instead demand a bar in every contract separately. Both arms treated identically — real corrected against its own shuffled twin per contract, control corrected against a SECOND independent shuffle. **Real: 17 survivors. Shuffled control: 2,085.** Ratio 0.01x, and it holds at EVERY bar: +0.0pp 17 vs 2,085; +0.5pp 2 vs 330; +1.0pp 0 vs 56. The real tape does not merely fail to beat the noise floor, it comes in at one percent of it. Cause is real rather than instrumental: **volatility clustering.** A stop sized from a trailing median swing is too tight exactly when volatility expands, so stop-outs run hot on a real tape and not on a shuffled one, and every strategy in this family is scaled that way. First version of this test filtered on RAW above-geometry and returned real 0 / shuffled 33,818 — caught only because the power curve was lowered to a bar of merely "positive", where chance demands thousands |

**THE STOP-SIZING LESSON, which is the one transferable thing here.** Across 1.2M strategies the real tape underperformed a volatility-matched random walk by two orders of magnitude in survivor count. That is not the market being efficient — a shuffled tape is equally efficient. It is the strategies being mis-specified: trailing-median stop sizing is systematically punished by volatility clustering, because the estimate lags the regime it is sizing for. Any future work should size stops from CURRENT realised volatility, not a trailing median, and should verify that choice against a shuffled tape where clustering does not exist.
