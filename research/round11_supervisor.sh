#!/bin/bash
# Supervisor for the round11 strategy search.
# Restarts the python process whenever it dies, resuming from checkpoint.
# Usage:
#   nohup ./research/round11_supervisor.sh [offset] [suffix] [max_days] > /tmp/round11_supervisor.log 2>&1 &
# Stop:
#   pkill -f round11_supervisor.sh && pkill -f round11_search.py
set -u

SCRIPT="/home/user/HFTBot/research/round11_search.py"
OFFSET="${1:-7820974790}"
SUFFIX="${2:-}"
MAX_DAYS="${3:-60}"
RESULTS="/home/user/HFTBot/research/round11_results.md"
CHECKPOINT="/home/user/HFTBot/research/round11_checkpoint${SUFFIX}.pkl"
RUN_LOG="/tmp/round11_run${SUFFIX}.log"
MAX_BACKOFF_S=60
...
while true; do
    ...
    if [ ! -f "$CHECKPOINT" ] && [ -f "$RESULTS" ]; then
        ...
        if [ "$results_age" -lt 600 ]; then
            echo "[$(date '+%Y-%m-%d %H:%M:%S')] supervisor: round11 completed successfully. Exiting."
            exit 0
        fi
    fi
