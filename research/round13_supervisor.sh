#!/bin/bash
# Supervisor for the round13 strategy search.
# Restarts the python process whenever it dies, resuming from checkpoint.
# Usage:
#   nohup ./research/round13_supervisor.sh [offset] [suffix] [max_days] > /tmp/round13_supervisor.log 2>&1 &
# Stop:
#   pkill -f round13_supervisor.sh && pkill -f round13_search.py
set -u

SCRIPT="/home/user/HFTBot/research/round13_search.py"
OFFSET="${1:-7820974790}"
SUFFIX="${2:-}"
MAX_DAYS="${3:-60}"
SCALE="${4:-1.0}"
...
while true; do
    ...
    if [ ! -f "$CHECKPOINT" ] && [ -f "$RESULTS" ]; then
        ...
        if [ "$results_age" -lt 600 ]; then
            echo "[$(date '+%Y-%m-%d %H:%M:%S')] supervisor: round13 completed successfully. Exiting."
            exit 0
        fi
    fi
