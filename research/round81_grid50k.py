...
SRC = {}
for off in (0.0, 0.5, -0.5, 1.0, -1.0, 1.5, -1.5, 2.0, -2.0, 3.0, -3.0):
    SRC[f"PREV{off:+.1f}"] = M["S"].shift(1) + off
SRC["PIN+0"] = M["pin"].shift(1)
SRC["PIN+1"] = M["pin"].shift(1) + 1.0
SRC["PIN-1"] = M["pin"].shift(1) - 1.0
S1 = M["S"].shift(1)

CFGS = []
for tg in (25., 40., 60., 85., 115., 150., 200., 260.):
    for st_ in (10., 16., 24., 34., 46., 60., 80.):
...
for src, lv in SRC.items():
    ...
    for ci in order:
        ...
        if shown >= 15:
            break
    np.savez(f"research/round81_{src}.npz", usd_wk=usd_wk,
