"""Cumulative-Delta Order Flow Strategy with REALISTIC execution from day 1.

WHY THIS MIGHT WORK WHERE LARGE-PRINT FAILED:
  Large-print strategy relied on reacting in microseconds — HFT eats that.
  Cumulative delta BUILDS UP over 30+ seconds. By the time signal fires,
  the trend is well-established. 200ms latency is irrelevant.

STRATEGY:
  - Compute rolling cumulative delta over WINDOW_SECS seconds
  - When |delta| > DELTA_THRESHOLD AND price hasn't already moved too far,
    enter in the direction of the flow
  - Hold with trailing stop
  - Cooldown between trades

REALISM FROM THE START:
  - 200ms latency on entry
  - 0.25pt slippage on entries AND exits
  - $1.00/contract round-trip commission ($0.50 each side — Tradovate rate)
"""
...
def run_cumdelta(price, ts_ns, signed_vol,
                 window_secs: int,
                 delta_thresh: int,
                 init_stop_pts: float,
    ...
    """Returns array of P&L per trade."""
    n = len(price)
    ...
    # Rolling cumulative delta using two-pointer approach
    pnls = []
    last_exit_ts = -1
    cumdelta = 0
    win_start = 0
    ...
    # We need to track cumdelta efficiently. Pre-sum signed_vol cumulatively.
    # Then rolling window = cumsum[i] - cumsum[j] where j is first tick within window.
    cum_signed = np.zeros(n + 1, dtype=np.int64)
    cum_signed[1:] = np.cumsum(signed_vol.astype(np.int64))

    for i in range(n - 1):
        # advance window start
        while win_start <= i and ts_ns[i] - ts_ns[win_start] > window_ns:
            win_start += 1
        if win_start >= i:
            continue
        window_delta = int(cum_signed[i + 1] - cum_signed[win_start])

        # cooldown check
        if last_exit_ts > 0 and ts_ns[i] - last_exit_ts < cd_ns:
            continue

        # entry signal: |delta| exceeds threshold
        if abs(window_delta) < delta_thresh:
            continue
        if active: continue   # only one position at a time

        side = 1 if window_delta > 0 else -1

        # LATENCY: find first tick after trigger + latency_ns
        ...
        for j in range(entry_idx, end_idx + 1):
            p = float(price[j])
            if side == 1 and p > best_fav:
                best_fav = p
            elif side == -1 and p < best_fav:
                best_fav = p
            if side == 1:
...
def summarize(pnls, period_days):
    ...
    return {
        ...
        "trades_per_week": n*5/period_days,
...
def main():
    ...
    del df
    print(f"  {len(price):,} ticks, {period_days:.0f} days\n")

    print(f"{'Win':>5} {'Thrsh':>6} {'Stop':>5} {'Tract':>5} {'Trdist':>6} {'Hold':>5} {'CD':>4} "
          f"{'n':>5} {'/mo':>4} {'/d':>4} {'WR':>6} {'PF':>5} {'RR':>5} "
            ...
            for istop in [1.0, 2.0, 3.0]:
                for tract in [1.0, 2.0]:
                    for trdist in [0.5, 1.0]:
                        for hold in [60, 180]:
                            cfg_count += 1
                            pnls = run_cumdelta(price, ts_ns, signed_vol,
                            ...
                            print(f"{window:>4}s {thresh:>6} {istop:>5.1f} "
                                  f"{tract:>5.1f} {trdist:>5.1f} {hold:>4}s "
                                  f"{10:>3}s {s['n']:>5} {s['trades_per_mo']:>4.0f} "
                                  f"{s['trades_per_day']:>3.1f} "
    ...
    print("\n" + "=" * 100)
    print("TOP 15 BY RET/MO  (all with REALISTIC latency+slippage+$1 comm)")
    print("=" * 100)
    ...
    for r in results[:15]:
        ...
        print(f"  win={r['window']:>3}s thrsh={r['thresh']:>3} istop={r['istop']:.1f} "
              ...
              f"n={r['n']:>4} ({r['trades_per_mo']:.0f}/mo, {r['trades_per_day']:.1f}/day) "
