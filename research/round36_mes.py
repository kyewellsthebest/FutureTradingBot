"""Round 36: MES -> NQ cross-market battery, EXTENDED. True MES tick data
(Mar 30 - Jul 8 from the week-ticks release, user-triggered fetch).
Signals from ES on a 1-second grid; NQ entries tick-walked as always,
flat 1 MNQ, $1.50/RT. Select Apr 1 - May 31, blind Jun 1 - Jul 6 (full-length,
MES ticks from the week-ticks release; NQ gap Jun 19-27 noted).
...
def main():
    ...
    import glob as _g
    mes = pd.concat([pd.read_parquet(f)[["ts","price"]] for f in sorted(_g.glob("/tmp/claude-0/westicks/MESU6_2026*.parquet")) if pd.read_parquet(f).shape[0]>0], ignore_index=True).sort_values("ts")
    es = pd.DataFrame({"ts": mes["ts"].to_numpy(), "close": mes["price"].to_numpy()})
    es = es[(es["ts"] >= SEL_LO.value) & (es["ts"] < END.value)]
    es = es.drop_duplicates(subset="ts", keep="last")
