...
CFGS = []
for tg in (10., 15., 20., 30., 45., 60., 80., 110., 150., 200., 250., 300.):
    for st_ in (5., 8., 12., 18., 25., 35., 50., 65., 80., 100.):
        for hd in (5, 15, 30, 60, 120, 240):
            for cf in (0, 60):
                for dr in ("b", "s", "l"):
                    for gt in ("all", "st", "sl"):
...
for src, lv in SRC.items():
    ...
    for ci in order:
        ...
        if shown >= 15:
            break
    np.savez(f"research/round80_{src}.npz", usd_wk=usd_wk,
